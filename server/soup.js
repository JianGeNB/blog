var Koa = require('koa'),
    ...
    comment = commentModule.getModel('comments'),
    bodyParser = require('koa-bodyparser'),
    path = require('path'),
    sendfile = require('koa-send'),
...
router.get('/checkenSoup',async(ctx,next)=>{
    ctx.body = {code:0,soupList}
})
router.get('/comment',async(ctx,next)=>{
    let infos;
    await comment.find({},function(err,doc){
        infos = doc
        ctx.body = {code:0,commitList:infos}
    })
})
router.post('/add',async(ctx)=>{
    const info = ctx.request.body;
    console.log(info)
     comment.create(info).then(
            ctx.body={code:0,msg:'评论成功'}
        )
})
// app.use(async(ctx,next)=> {
//     if(ctx.url.startsWith('/checkenSoup'||ctx.url.startsWith('/static/'))){
//         return await next()
//     }
//     return sendfile(this,'../build/index.html')
// })
// // app.use('/',Koa.static(path.resolve('build')))
// app.use(static(
//     path.join('../build')
// )) 
app
